...
int main(int argc, char *argv[]){
    ...
    sh->init();
	// sh->connectSfuServer();
	if(sh->connectSfuServer() != Yang_Ok){
		std::cout << "sh->connectSfuServer() failed" << std::endl;
	}else{
		std::cout << "sh->connectSfuServer() successed" << std::endl;
	}
  
    sleep(1);
	// configContext.streams.sendRequest(0,0,Yang_Req_Connected);
	configContext.streams.connectNotify(streamconfig.uid,streamconfig.streamOptType,true);
